...
import org.apache.hadoop.hbase.filter.{BinaryComparator, CompareFilter, Filter, RowFilter}
/**
...
object HBaseLocalTest {
  ...
  def createHTable(connection: Connection,tablename: String): Unit=
  {
    ...
    if (!admin.tableExists(tableName)) {
      ...
      //创建列簇1    artitle
      tableDescriptor.addFamily(new HColumnDescriptor("master_data".getBytes()))
//      tableDescriptor.addFamily(new HColumnDescriptor("identification".getBytes()))
//      tableDescriptor.addFamily(new HColumnDescriptor("name".getBytes()))
//      tableDescriptor.addFamily(new HColumnDescriptor("industry".getBytes()))
//      tableDescriptor.addFamily(new HColumnDescriptor("date".getBytes()))
//      tableDescriptor.addFamily(new HColumnDescriptor("city".getBytes()))
//      tableDescriptor.addFamily(new HColumnDescriptor("nowDate".getBytes()))
      //      tableDescriptor.addFamily(new HColumnDescriptor("occupation".getBytes()))
      //创建表
  ...
  //  val filter1: Filter = new RowFilter(CompareFilter.CompareOp.LESS_OR_EQUAL, new BinaryComparator(("2"+"-"+"1:2"+"-").getBytes))
  //  s.setFilter(filter1)
  //  val scanner1 = table.getScanner(s)
  //  println(scanner1)
...
//    conf.set("hbase.master","hbase:60000")
    ...
    try{
      ...
      //创建表测试
      try {
        createHTable(connection, "porsche")

//        getAResult(connection,"porscheText","master_data","yichang","18352866716-360")
        //插入数据,重复执行为覆盖
//                insertHTable(connection,"porscheText","master_data","zhangrenjie","001","1")
        ...
        //扫描整个表
        //scanRecord(connection,"porsche","name","zhangrenjie")
        //        scanRecord(connection,"blog","artitle","engish")
        //删除表测试
//         deleteHTable(connection, "porsche")
      }finally {
        connection.close
        //   sc.stop
      }
